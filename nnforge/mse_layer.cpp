...
namespace nnforge
{
	...
	layer_configuration mse_layer::get_layer_configuration(const std::vector<layer_configuration>& input_configuration_list) const
	{
		...
		if ((input_configuration_list.size() > 2) && (input_configuration_list[2].feature_map_count >= 0) && (input_configuration_list[2].feature_map_count != 1))
			throw neural_network_exception((boost::format("Feature map count for mse_layer scaling should be equal to 1, while it is %1%") % input_configuration_list[2].feature_map_count).str());
