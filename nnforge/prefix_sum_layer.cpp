...
namespace nnforge
{
	const std::string prefix_sum_layer::layer_type_name = "PrefixSum";

	prefix_sum_layer::prefix_sum_layer(
	...
	void prefix_sum_layer::check()
	{
		if (feature_map_segment_length < 2)
			throw neural_network_exception("feature_map_segment_length for prefix sum layer should be at least 2");

		if (clamp_min >= clamp_max)
			throw neural_network_exception("clamp_min should be smaller than clamp_max for prefix sum layer");
	}

	std::string prefix_sum_layer::get_type_name() const
	{
		return layer_type_name;
	}

	layer::ptr prefix_sum_layer::clone() const
	{
		return layer::ptr(new prefix_sum_layer(*this));
	}

	layer_configuration prefix_sum_layer::get_layer_configuration(const std::vector<layer_configuration>& input_configuration_list) const
	{
	...
	layer_configuration_specific prefix_sum_layer::get_output_layer_configuration_specific(const std::vector<layer_configuration_specific>& input_configuration_specific_list) const
	{
	...
	bool prefix_sum_layer::get_input_layer_configuration_specific(
	...
	void prefix_sum_layer::write_proto(void * layer_proto) const
	{
		protobuf::Layer * layer_proto_typed = reinterpret_cast<nnforge::protobuf::Layer *>(layer_proto);
		nnforge::protobuf::PrefixSumParam * param = layer_proto_typed->mutable_prefix_sum_param();
	...
	void prefix_sum_layer::read_proto(const void * layer_proto)
	{
		const protobuf::Layer * layer_proto_typed = reinterpret_cast<const nnforge::protobuf::Layer *>(layer_proto);
		if (!layer_proto_typed->has_prefix_sum_param())
			throw neural_network_exception((boost::format("No prefix_sum_param specified for layer %1% of type %2%") % instance_name % layer_proto_typed->type()).str());
		const protobuf::PrefixSumParam& param = layer_proto_typed->prefix_sum_param();
	...
	float prefix_sum_layer::get_flops_per_entry(
	...
	std::vector<std::string> prefix_sum_layer::get_parameter_strings() const
	{
