...
namespace nnforge
{
	namespace plain
	{
		std::string absolute_layer_updater_plain::get_type_name() const
		{
			return absolute_layer::layer_type_name;
		}

		void absolute_layer_updater_plain::run_forward_propagation(
		...
		void absolute_layer_updater_plain::run_backward_data_propagation(
			...
			unsigned int entry_count) const
		{
			...
			if (add_update_to_destination)
			{
				...
				for(int i = 0; i < elem_count; ++i)
				{
					...
					*(in_err_it + i) += (input_val < 0.0F) ? -out_err : out_err;
				}
			}
			else
			{
		...
		int absolute_layer_updater_plain::get_input_index_layer_can_write(
		...
		bool absolute_layer_updater_plain::is_backward_data_dependent_on_input_buffer(
		...
		bool absolute_layer_updater_plain::is_backward_data_dependent_on_output_buffer(
			unsigned int action_input_index,
			const std::set<layer_action>& actions,
			plain_running_configuration::const_ptr plain_config,
			layer::const_ptr layer_schema,
