...
namespace nnforge
{
	namespace plain
	{
		...
		void upsampling_layer_updater_plain::run_forward_propagation(
		...
		void upsampling_layer_updater_plain::run_backward_data_propagation(
			...
			unsigned int entry_count) const
		{
			...
			#pragma omp parallel default(none) num_threads(plain_config->openmp_thread_count)
			{
				...
				for(int workload_id = 0; workload_id < total_workload; ++workload_id)
				{
					...
					float * in_err_it_base = in_err_it_global + (input_entry_id * input_neuron_count) + (input_feature_map_id * input_neuron_count_per_feature_map);
					const float * out_err_it_base = out_err_it_global + (input_entry_id * entry_upsampling_size * output_neuron_count ) + (input_feature_map_id * feature_map_upsampling_size * output_neuron_count_per_feature_map);

					std::fill_n(current_input_position.begin(), spatial_dimension_count, 0);
					for(float * in_it = in_err_it_global; in_it != in_err_it_global + input_neuron_count_per_feature_map; ++in_it)
					{
						// Define the starting position of the first output elem
						const float * out_it = out_err_it_base;
						for(unsigned int i = 0; i < spatial_dimension_count; ++i)
							out_it += current_input_position[i] * (*(upsampling_sizes_it + i)) * (*(output_slices_it + i));

						float err = 0.0F;
						for(unsigned int i = 0; i < const_upsampling_elem_count; ++i)
							err += *(out_it + (*(offset_list_it + i)));
		...
		bool upsampling_layer_updater_plain::is_backward_data_dependent_on_input_buffer(
		...
		bool upsampling_layer_updater_plain::is_backward_data_dependent_on_output_buffer(
			unsigned int action_input_index,
			const std::set<layer_action>& actions,
			plain_running_configuration::const_ptr plain_config,
			layer::const_ptr layer_schema,
