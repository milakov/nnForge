...
namespace nnforge
{
	namespace plain
	{
		std::string exponential_linear_layer_tester_plain::get_type_name() const
		{
			return exponential_linear_layer::layer_type_name;
		}

		void exponential_linear_layer_tester_plain::run_forward_propagation(
			...
			unsigned int entry_count) const
		{
			...
			for(int i = 0; i < elem_count; ++i)
			{
				...
				*(out_it + i) = input_val >= 0.0F ? input_val : (expf(input_val) - 1.0F);
			}
		}

		int exponential_linear_layer_tester_plain::get_input_index_layer_can_write(
			plain_running_configuration::const_ptr plain_config,
			layer::const_ptr layer_schema,
			const std::vector<layer_configuration_specific>& input_configuration_specific_list,
