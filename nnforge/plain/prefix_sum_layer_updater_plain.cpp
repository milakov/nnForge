...
namespace nnforge
{
	namespace plain
	{
		prefix_sum_layer_updater_plain::prefix_sum_layer_updater_plain()
		{
		}
		...
		std::string prefix_sum_layer_updater_plain::get_type_name() const
		{
			return prefix_sum_layer::layer_type_name;
		}

		void prefix_sum_layer_updater_plain::run_forward_propagation(
		...
		void prefix_sum_layer_updater_plain::run_backward_data_propagation(
			...
			unsigned int entry_count) const
		{
			...
			#pragma omp parallel default(none) num_threads(plain_config->openmp_thread_count)
			{
				...
				for(int workload_id = 0; workload_id < total_workload; ++workload_id)
				{
					...
					for(unsigned int i = 0; i < feature_map_segment_length; ++i, offset -= neuron_count_per_feature_map)
					{
						running_sum += out_err_it_global[offset];
						in_err_it_global[offset] = running_sum;
					}
				}
			}
		}

		int prefix_sum_layer_updater_plain::get_input_index_layer_can_write(
		...
		bool prefix_sum_layer_updater_plain::is_backward_data_dependent_on_input_buffer(
		...
		bool prefix_sum_layer_updater_plain::is_backward_data_dependent_on_output_buffer(
			unsigned int action_input_index,
			const std::set<layer_action>& actions,
			plain_running_configuration::const_ptr plain_config,
			layer::const_ptr layer_schema,
