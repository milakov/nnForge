...
namespace nnforge
{
	namespace plain
	{
		std::string exponential_linear_layer_updater_plain::get_type_name() const
		{
			return exponential_linear_layer::layer_type_name;
		}

		void exponential_linear_layer_updater_plain::run_forward_propagation(
			...
			unsigned int entry_count) const
		{
			...
			{
				float inp = *(in_it + i);
				float res = inp >= 0.0F ? inp : (expf(inp) - 1.0F);
				*(out_it + i) = res;
			}
		}

		void exponential_linear_layer_updater_plain::run_backward_data_propagation(
			...
			unsigned int entry_count) const
		{
			...
			if (add_update_to_destination)
			{
				...
				for(int i = 0; i < elem_count; ++i)
				{
					...
					float der1st = (out_neuron >= 0) ? 1.0F : (out_neuron + 1.0F);
		...
		int exponential_linear_layer_updater_plain::get_input_index_layer_can_write(
		...
		bool exponential_linear_layer_updater_plain::is_backward_data_dependent_on_input_buffer(
		...
		bool exponential_linear_layer_updater_plain::is_backward_data_dependent_on_output_buffer(
			unsigned int action_input_index,
			const std::set<layer_action>& actions,
			plain_running_configuration::const_ptr plain_config,
			layer::const_ptr layer_schema,
