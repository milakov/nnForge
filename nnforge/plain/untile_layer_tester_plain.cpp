...
namespace nnforge
{
	namespace plain
	{
		std::string untile_layer_tester_plain::get_type_name() const
		{
			return untile_layer::layer_type_name;
		}

		void untile_layer_tester_plain::run_forward_propagation(
			...
			unsigned int entry_count) const
		{
			...
			#pragma omp parallel default(none) num_threads(plain_config->openmp_thread_count)
			{
				...
				for(int workload_id = 0; workload_id < total_workload; ++workload_id)
				{
					...
					float * base_output_it = out_it_global + (output_entry_id * feature_map_count + feature_map_id) * output_neuron_count_per_feature_map;
					const float * base_input_it = in_it_global + (base_input_entry_id * feature_map_count + feature_map_id) * input_neuron_count_per_feature_map;

					for(int input_neuron_id = 0; input_neuron_id < static_cast<int>(input_neuron_count_per_feature_map); ++input_neuron_id)
					{
						float * base_output_it2 = base_output_it + position_list_it[input_neuron_id];
						const float * base_input_it2 = base_input_it + input_neuron_id;
