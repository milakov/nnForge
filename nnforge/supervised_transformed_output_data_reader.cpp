...
namespace nnforge
{
	...
	void supervised_transformed_output_data_reader::next_epoch()
	{
		current_sample_id = 0;
		transformer->reset();
	...
	unsigned int supervised_transformed_output_data_reader::get_entry_count() const
	{
	...
	void supervised_transformed_output_data_reader::rewind(unsigned int entry_id)
	{
		throw std::runtime_error("rewind not implemented for supervised_transformed_output_data_reader");
	}

	bool supervised_transformed_output_data_reader::raw_read(std::vector<unsigned char>& all_elems)
	{
