...
namespace nnforge
{
		...
		const std::vector<unsigned int>& snapshot_data_dimension_list)
	{
		...
		for(unsigned int i = 0; i < config.dimension_sizes.size(); ++i)
		{
			if (i == snapshot_data_dimension_list[0])
				x_step = current_step;
			if (i == snapshot_data_dimension_list[1])
				y_step = current_step;
			current_step *= config.dimension_sizes[i];
		}

		unsigned int image_width = config.dimension_sizes[snapshot_data_dimension_list[0]];
		unsigned int image_height = config.dimension_sizes[snapshot_data_dimension_list[1]];

		float addition = 0.0F;
		float mult = 255.0F;
		{
			float min_val = *std::min_element(data, data + config.get_neuron_count());
			float max_val = *std::max_element(data, data + config.get_neuron_count());
			if (min_val >= max_val)
			{
		...
		if (is_rgb)
		{
			...
			{
				const float * red_it = data;
				const float * green_it = data + config.get_neuron_count_per_feature_map();
				const float * blue_it = data + config.get_neuron_count_per_feature_map() * 2;
		...
		{
			cv::Mat1b im(image_height, image_width * config.feature_map_count + config.feature_map_count - 1);
			im = 0;
			{
				const float * it = data;
				for(unsigned int feature_map_id = 0; feature_map_id < config.feature_map_count; ++feature_map_id, it += config.get_neuron_count_per_feature_map())
				{
		...
		const std::vector<unsigned int>& snapshot_data_dimension_list)
	{
		...
		unsigned int video_length = config.dimension_sizes[snapshot_data_dimension_list[0]];
		unsigned int video_width = config.dimension_sizes[snapshot_data_dimension_list[1]];
		unsigned int video_height = config.dimension_sizes[snapshot_data_dimension_list[2]];
		...
		{
			cv::Mat1b im(video_height, video_width * config.feature_map_count + config.feature_map_count - 1);
			im = 0;
			cv::Mat1b dst_image(im.rows * scale, im.cols * scale);
	...
	/*
	void data_visualizer::save_snapshot_video(
	...
	void data_visualizer::save_ann_snapshot(
