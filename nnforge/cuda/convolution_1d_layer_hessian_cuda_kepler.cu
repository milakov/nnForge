...
namespace nnforge
{
	namespace cuda
	{
		...
		__global__ void convolution_1d_tex_blocked_hess_kernel_kepler(
		...
		__global__ void convolution_1d_tex_exact_blocked_hess_kernel_kepler(
			float * __restrict output,
			cudaTextureObject_t input_tex,
			cudaTextureObject_t weights_tex,
			const float * __restrict biases,
		...
		__global__ void convolution_1d_update_biases_hess_kernel_kepler(
		...
		template<int BLOCK_SIZE>
		__global__ void convolution_1d_square_deriviative_tex_hess_kernel_kepler(
			...
			int entry_count)
		{
			...
			if (in_bounds)
			{
				...
				int weights_offset = window_width * input_feature_map_id;
				...
				for(int output_layer_id = 0; output_layer_id < output_feature_map_count; ++output_layer_id)
				{
					...
					weights_offset += window_width * (input_feature_map_count - 1);
		...
		template<int WINDOW_WIDTH, int BLOCK_SIZE>
		__global__ void convolution_1d_square_deriviative_tex_exact_hess_kernel_kepler(
			float * __restrict input_errors,
			cudaTextureObject_t output_tex,
			cudaTextureObject_t weights_squared_tex,
			int output_width,
		...
		__global__ void convolution_1d_update_weights_hess_kernel_kepler(
			...
			int block_size)
		{
			...
			if ((weight_x < window_width) && (output_feature_map_id < output_feature_map_count) && (base_entry_id < entry_count))
			{
				...
				int output_errors_offset = (base_entry_id * output_feature_map_count + output_feature_map_id) * output_width;
				...
				for(int t = 0; t < iteration_count; ++t)
				{
					...
					output_errors_offset += (output_feature_map_count - 1) * output_width;
		...
		__global__ void convolution_1d_update_weights_exact_hess_kernel_kepler(
			float * __restrict hessian_weights,
			cudaTextureObject_t input_squared_tex,
			cudaTextureObject_t output_tex,
			int output_width,
		...
		convolution_1d_layer_hessian_cuda_kepler::convolution_1d_layer_hessian_cuda_kepler()
		{
		}
...
#define launch_exact_kernel_const_const(window_width_const, block_size_const) \
	convolution_1d_tex_exact_blocked_hess_kernel_kepler<window_width_const,block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*output_neurons_buffer, input_tex, weights_tex, *data[1], output_configuration_specific.dimension_sizes[0], input_configuration_specific.dimension_sizes[0], input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, entry_count);
...
#define launch_kernel_const(block_size_const) \
	convolution_1d_tex_blocked_hess_kernel_kepler<block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*output_neurons_buffer, input_tex, weights_tex, *data[1], output_configuration_specific.dimension_sizes[0], input_configuration_specific.dimension_sizes[0], window_sizes[0], input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, entry_count);
...
#define launch_backprop_exact_kernel_const_const(window_width_const, block_size_const) \
	convolution_1d_square_deriviative_tex_exact_hess_kernel_kepler<window_width_const,block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*input_errors_buffer, output_tex, weights_squared_tex, output_configuration_specific.dimension_sizes[0], input_configuration_specific.dimension_sizes[0], input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, entry_count);
...
#define launch_backprop_kernel_const(block_size_const) \
	convolution_1d_square_deriviative_tex_hess_kernel_kepler<block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*input_errors_buffer, output_tex, weights_squared_tex, output_configuration_specific.dimension_sizes[0], input_configuration_specific.dimension_sizes[0], window_sizes[0], input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, entry_count);
...
#define launch_update_weights_exact_kernel_const(window_width_const) \
	convolution_1d_update_weights_exact_hess_kernel_kepler<window_width_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*hessian_data[0], input_squared_tex, output_tex, output_configuration_specific.dimension_sizes[0], input_configuration_specific.dimension_sizes[0], input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, entry_count, block_size);
...
#define launch_update_weights_exact_kernel(window_width) \
	switch (window_width) \
		...
		void convolution_1d_layer_hessian_cuda_kepler::enqueue_test(
		...
		void convolution_1d_layer_hessian_cuda_kepler::enqueue_backprop(
			cudaStream_t stream_id,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& schema_data,
		...
		void convolution_1d_layer_hessian_cuda_kepler::enqueue_update_hessian(
			cudaStream_t stream_id,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& schema_data,
			...
			unsigned int entry_count)
		{
			// Update weights
			{
				...
				else
				{
					...
					convolution_1d_update_weights_hess_kernel_kepler<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
						*hessian_data[0],
						input_squared_tex,
						output_tex,
			...
			// Update biases
			{
				...
				convolution_1d_update_biases_hess_kernel_kepler<<<kernel_dims.first, kernel_dims.second, smem_size, stream_id>>>(
					*hessian_data[1],
		...
		int convolution_1d_layer_hessian_cuda_kepler::get_block_size(int width)
		{
		...
		void convolution_1d_layer_hessian_cuda_kepler::hessian_configured()
		{
		...
		bool convolution_1d_layer_hessian_cuda_kepler::is_in_place_backprop() const
		{
			return false;
		}

		std::vector<size_t> convolution_1d_layer_hessian_cuda_kepler::get_sizes_of_additional_buffers_per_entry() const
		{
		...
		std::vector<unsigned int> convolution_1d_layer_hessian_cuda_kepler::get_linear_addressing_through_texture_per_entry() const
		{
		...
		int convolution_1d_layer_hessian_cuda_kepler::get_bias_update_block_size(int entry_count)
		{
			int block_size = std::min<int>(std::max<int>(static_cast<int>(sqrtf(static_cast<float>(entry_count))), 1), entry_count);
			return block_size;
		}

		int convolution_1d_layer_hessian_cuda_kepler::get_weights_update_block_size(int entry_count)
		{
			int block_size = std::min<int>(std::max<int>(static_cast<int>(sqrtf(static_cast<float>(entry_count))), 1), entry_count);
			return block_size;
		}
