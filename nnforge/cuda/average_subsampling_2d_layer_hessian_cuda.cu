...
__global__ void average_subsampling_2d_tex_hess_kernel(
...
__global__ void average_subsampling_2d_tex_exact_hess_kernel(
	float * __restrict output,
	int input_width,
	int input_height,
	int output_width,
...
__global__ void average_subsampling_2d_square_deriviative_hess_kernel(
	...
	int entry_count)
{
	...
	if (in_bounds)
	{
		error = output_errors[((entry_id * feature_map_count + feature_map_id) * output_height + output_y) * output_width + output_x] * subsampling_weight_squared;
...
template<int SUBSAMPLING_WIDTH, int SUBSAMPLING_HEIGHT>
__global__ void average_subsampling_2d_square_deriviative_exact_hess_kernel(
	float * __restrict input_errors,
	const float * __restrict output_errors,
	int input_width,
	int input_height,
	...
	int entry_count)
{
	...
	if (in_bounds)
	{
		error = output_errors[((entry_id * feature_map_count + feature_map_id) * output_height + output_y) * output_width + output_x] * ((1.0F / (float)(SUBSAMPLING_WIDTH * SUBSAMPLING_HEIGHT)) * (1.0F / (float)(SUBSAMPLING_WIDTH * SUBSAMPLING_HEIGHT)));
		current_input_errors_elem_id = ((entry_id * feature_map_count + feature_map_id) * input_height + input_y) * input_width + input_x;
	}
...
namespace nnforge
{
	namespace cuda
	{
		average_subsampling_2d_layer_hessian_cuda::average_subsampling_2d_layer_hessian_cuda()
		{
...
#define launch_backprop_exact_kernel_const_const(window_width_const, window_height_const) \
	average_subsampling_2d_square_deriviative_exact_hess_kernel<window_width_const,window_height_const><<<kernel_dims.first, kernel_dims.second, smem_size, stream_id>>>(*input_errors_buffer,*output_errors_buffer,input_configuration_specific.dimension_sizes[0],input_configuration_specific.dimension_sizes[1],output_configuration_specific.dimension_sizes[0],output_configuration_specific.dimension_sizes[1],output_configuration_specific.feature_map_count,entry_count);
...
#define launch_backprop_exact_kernel(window_width, window_height) \
	switch (window_height) \
		...
		void average_subsampling_2d_layer_hessian_cuda::enqueue_test(
			...
			unsigned int entry_count)
		{
			...
			{
				average_subsampling_2d_tex_hess_kernel<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
		...
		void average_subsampling_2d_layer_hessian_cuda::enqueue_backprop(
			cudaStream_t stream_id,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& schema_data,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& data,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& data_custom,
			...
			unsigned int entry_count)
		{
			...
			{
				average_subsampling_2d_square_deriviative_hess_kernel<<<kernel_dims.first, kernel_dims.second, smem_size, stream_id>>>(
					*input_errors_buffer,
					*output_errors_buffer,
					subsampling_sizes[0],
					subsampling_sizes[1],
					subsampling_weight_squared,
					input_configuration_specific.dimension_sizes[0],
					input_configuration_specific.dimension_sizes[1],
					output_configuration_specific.dimension_sizes[0],
					output_configuration_specific.dimension_sizes[1],
		...
		std::vector<unsigned int> average_subsampling_2d_layer_hessian_cuda::get_linear_addressing_through_texture_per_entry() const
		{
