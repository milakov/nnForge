...
namespace nnforge
{
	namespace cuda
	{
		...
		__global__ void max_subsampling_hess_kernel(
		...
		__global__ void max_subsampling_bbprop_hess_kernel(
		...
		template<int dimension_count>
		class max_subsampling_layer_hessian_cuda : public layer_hessian_cuda
		{
		public:
			max_subsampling_layer_hessian_cuda()
			{
			}

			virtual ~max_subsampling_layer_hessian_cuda()
			{
			}
				...
				unsigned int entry_count)
			{
				...
				max_subsampling_hess_kernel<<<kernel_dims.first, kernel_dims.second, smem_size, stream_id>>>(
					output,
					max_positions,
					input,
					packed_config_list,
				...
				unsigned int entry_count)
			{
				...
				max_subsampling_bbprop_hess_kernel<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
					input_errors,
					max_positions,
					output_errors,
					elem_count);
