...
__global__ void maxout_square_deriviative_hess_kernel(
...
namespace nnforge
{
	namespace cuda
	{
		maxout_layer_hessian_cuda::maxout_layer_hessian_cuda()
		{
		}
		...
		void maxout_layer_hessian_cuda::enqueue_test(
			...
			unsigned int entry_count)
		{
			...
			maxout_hess_kernel<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
				output,
		...
		void maxout_layer_hessian_cuda::enqueue_backprop(
			cudaStream_t stream_id,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& schema_data,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& data,
			...
			unsigned int entry_count)
		{
			...
			maxout_square_deriviative_hess_kernel<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
				input_errors,
				max_feature_map_positions,
				output_errors,
				output_elem_count_per_feature_map,
		...
		bool maxout_layer_hessian_cuda::is_in_place_backprop() const
		{
			return false;
		}

		std::vector<size_t> maxout_layer_hessian_cuda::get_sizes_of_additional_buffers_per_entry() const
		{
			std::vector<size_t> res;

			res.push_back(output_elem_count_per_entry * sizeof(int));
