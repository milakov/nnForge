...
__global__ void convolution_1d_tex_blocked_kernel_kepler(
...
__global__ void convolution_1d_tex_exact_blocked_kernel_kepler(
	float * __restrict output,
	cudaTextureObject_t input_tex,
	cudaTextureObject_t weights_tex,
	const float * __restrict biases,
...
namespace nnforge
{
	namespace cuda
	{
		convolution_1d_layer_tester_cuda_kepler::convolution_1d_layer_tester_cuda_kepler()
		{
		}
...
#define launch_exact_kernel_const_const(window_width_const, block_size_const) \
	convolution_1d_tex_exact_blocked_kernel_kepler<window_width_const,block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*additional_buffers[0], input_tex, weights_tex, *data[1], output_configuration_specific.dimension_sizes[0], input_configuration_specific.dimension_sizes[0], input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, entry_count);
...
#define launch_kernel_const(block_size_const) \
	convolution_1d_tex_blocked_kernel_kepler<block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*additional_buffers[0], input_tex, weights_tex, *data[1], output_configuration_specific.dimension_sizes[0], input_configuration_specific.dimension_sizes[0], window_sizes[0], input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, entry_count);
...
#define launch_kernel(block_size) \
	switch (block_size) \
		...
		void convolution_1d_layer_tester_cuda_kepler::enqueue_test(
		...
		int convolution_1d_layer_tester_cuda_kepler::get_block_size(int output_width)
		{
		...
		void convolution_1d_layer_tester_cuda_kepler::tester_configured()
		{
		...
		std::vector<size_t> convolution_1d_layer_tester_cuda_kepler::get_sizes_of_additional_buffers_per_entry() const
		{
		...
		std::vector<unsigned int> convolution_1d_layer_tester_cuda_kepler::get_linear_addressing_through_texture_per_entry() const
		{
		...
		cuda_linear_buffer_device_smart_ptr convolution_1d_layer_tester_cuda_kepler::get_output_buffer(
			cuda_linear_buffer_device_smart_ptr input_buffer,
			const std::vector<cuda_linear_buffer_device_smart_ptr>& additional_buffers)
		{
