...
namespace nnforge
{
	namespace cuda
	{
		...
		texture<float2, cudaTextureType1D, cudaReadModeElementType> input_squared_tex_ref;

		template<int DIMENSION_COUNT, int BLOCK_SIZE>
		__launch_bounds__(256, 2)
		__global__ void convolution_tex_generic_blocked_hess_kernel_fermi(
			...
			int packed_config_count)
		{
			...
			if (in_bounds)
			{
				...
				const float2 * current_weights = weights + (int)(weight_count_per_output_feature_map * output_feature_map_id);
		...
		__launch_bounds__(256, 2)
		__global__ void convolution_tex_exact_blocked_hess_kernel_fermi(
			float * __restrict output,
			const float2 * __restrict weights,
			const float * __restrict biases,
			const packed_config<DIMENSION_COUNT> * __restrict packed_config_list,
			...
			int packed_config_count)
		{
			...
			if (in_bounds)
			{
				...
				const float2 * current_weights = weights_squared + (int)(weight_count_per_input_feature_map * input_feature_map_id);
		...
		__global__ void convolution_update_biases_hess_kernel_fermi(
		...
		__launch_bounds__(256, 2)
		__global__ void convolution_update_weights_exact_hess_kernel_fermi(
		...
		__launch_bounds__(256, 2)
		__global__ void convolution_update_weights_generic_hess_kernel_fermi(
			float * __restrict hessian_weights,
			const float2 * __restrict output_errors,
			const packed_config<DIMENSION_COUNT*2+2> * __restrict packed_config_list,
			array_by_val<int, DIMENSION_COUNT> output_sizes,
...
#define launch_exact_kernel_const_const_const(dimension_count_const, window_width_const, block_size_const) \
	convolution_tex_exact_blocked_hess_kernel_fermi<dimension_count_const,window_width_const,block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*output_neurons_buffer, *data[0], *data[1], packed_config_list, output_sizes, input_sizes, window_sizes, input_configuration_specific_striped.feature_map_count, output_configuration_specific.feature_map_count, entry_count, forward_packed_config_count);

#define launch_generic_kernel_const_const(dimension_count_const, block_size_const) \
	convolution_tex_generic_blocked_hess_kernel_fermi<dimension_count_const,block_size_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*output_neurons_buffer, *data[0], *data[1], packed_config_list, output_sizes, input_sizes, window_sizes, input_configuration_specific_striped.feature_map_count, output_configuration_specific.feature_map_count, entry_count, forward_packed_config_count);
...
#define launch_update_exact_kernel_const_const(dimension_count_const, window_width_const) \
	convolution_update_weights_exact_hess_kernel_fermi<dimension_count, window_width_const><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*hessian_data[0], output_errors, packed_config_list, output_sizes, input_sizes, window_sizes, input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, input_configuration_specific_striped.feature_map_count, output_configuration_specific_striped.feature_map_count, input_elem_count_per_entry_striped, output_elem_count_per_entry_striped, entry_count, block_size, updater_packed_config_count);

#define launch_update_generic_kernel_const(dimension_count_const) \
	convolution_update_weights_generic_hess_kernel_fermi<dimension_count><<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(*hessian_data[0], output_errors, packed_config_list, output_sizes, input_sizes, window_sizes, input_configuration_specific.feature_map_count, output_configuration_specific.feature_map_count, input_configuration_specific_striped.feature_map_count, output_configuration_specific_striped.feature_map_count, input_elem_count_per_entry_striped, output_elem_count_per_entry_striped, entry_count, block_size, updater_packed_config_count);
...
#define launch_update_kernel(dimension_count_const, window_width) \
	switch (window_width) \
		...
		template<int dimension_count>
		class convolution_layer_hessian_cuda_fermi : public layer_hessian_cuda
		{
		public:
			convolution_layer_hessian_cuda_fermi()
			{
			...
			virtual ~convolution_layer_hessian_cuda_fermi()
			{
			}
				...
				unsigned int entry_count)
			{
				...
				// Update weights
				{
					...
					cudaChannelFormatDesc desc = cudaCreateChannelDesc<float2>();
					cuda_safe_call(cudaBindTexture(0, input_squared_tex_ref, *additional_buffers[0], desc, additional_buffers[0]->get_size()));
				...
				// Update biases
				{
					...
					convolution_update_biases_hess_kernel_fermi<<<kernel_dims.first, kernel_dims.second, smem_size, stream_id>>>(
						*hessian_data[1],
						*output_errors_buffer,
						block_size,
						output_elem_count_per_feature_map,
		...
		private:
			...
			{
				int target_block_count = (10 * cuda_util::get_thread_count_per_wave(*cuda_config) / updater_packed_config_count) + 1;
				int target_block_size = (entry_count + target_block_count - 1) / target_block_count;
				target_block_count = (entry_count + target_block_size - 1) / target_block_size;
				int block_size = std::min(std::max(std::max(static_cast<int>(sqrtf(static_cast<float>(entry_count))), target_block_size), 1), entry_count);
				int block_count = (entry_count + block_size - 1) / block_size;
