			...
			const layer_configuration_specific& output_configuration_specific) const
		{
			...
			else
			{
				if (cuda_config->get_compute_capability() >= 300)
					...
					throw neural_network_exception("sparse convolutional layer has spatial fully connected tester implementation in CUDA backend for Kepler and above GPUs only");
			}
