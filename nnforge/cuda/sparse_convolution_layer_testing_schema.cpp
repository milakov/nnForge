	...
	namespace cuda
	{
			...
			const cuda_running_configuration& cuda_config) const
		{
			...
			if (zero_padding)
			{
				...
				{
					if (window1x1)
						return layer_tester_cuda::ptr(new sparse_fully_connected_1x1_layer_tester_cuda());
					else
						return layer_tester_cuda::ptr(new sparse_fully_connected_layer_tester_cuda());
				}
				else
				{
					if (window1x1)
			...
			if (unit_stride)
				return sparse_convolution_layer_testing_schema_helper_cuda::create_tester_specific(input_configuration_specific_list[0], output_configuration_specific);
