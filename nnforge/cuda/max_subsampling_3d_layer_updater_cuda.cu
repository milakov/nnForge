...
#include "max_subsampling_3d_layer_updater_cuda.h"
...
__global__ void max_subsampling_3d_tex_upd_kernel(
...
__global__ void max_subsampling_3d_deriviative_upd_kernel(
...
namespace nnforge
{
	namespace cuda
	{
		max_subsampling_3d_layer_updater_cuda::max_subsampling_3d_layer_updater_cuda()
		{
		}
		...
		void max_subsampling_3d_layer_updater_cuda::enqueue_test(
			...
			unsigned int entry_count)
		{
			...
			max_subsampling_3d_tex_upd_kernel<<<kernel_dims.first, kernel_dims.second, smem_size, stream_id>>>(
				output,
				max_positions,
				input,
		...
		void max_subsampling_3d_layer_updater_cuda::enqueue_backprop(
			cudaStream_t stream_id,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& schema_data,
			const std::vector<cuda_linear_buffer_device_smart_ptr>& data,
			...
			unsigned int entry_count)
		{
			...
			max_subsampling_3d_deriviative_upd_kernel<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
				input_errors,
				max_positions,
				output_errors,
				x_y_z_config_list,
		...
		void max_subsampling_3d_layer_updater_cuda::updater_configured()
		{
			if (!different_input)
				throw neural_network_exception("max_subsampling_3d_layer_updater_cuda is not able to run using the same input");
		...
		bool max_subsampling_3d_layer_updater_cuda::is_in_place_backprop() const
		{
			return false;
		}

		std::vector<size_t> max_subsampling_3d_layer_updater_cuda::get_sizes_of_additional_buffers_per_entry() const
		{
		...
		std::vector<size_t> max_subsampling_3d_layer_updater_cuda::get_sizes_of_additional_buffers_fixed() const
		{
			std::vector<size_t> res;
		...
		void max_subsampling_3d_layer_updater_cuda::fill_additional_buffers(const std::vector<cuda_linear_buffer_device_smart_ptr>& additional_buffers) const
		{
			{
