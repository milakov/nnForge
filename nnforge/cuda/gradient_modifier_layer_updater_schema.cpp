...
namespace nnforge
{
	namespace cuda
	{
		...
		std::string gradient_modifier_layer_updater_schema::get_type_name() const
		{
			return gradient_modifier_layer::layer_type_name;
		}

		layer_updater_cuda::ptr gradient_modifier_layer_updater_schema::create_updater_specific(
			const std::vector<layer_configuration_specific>& input_configuration_specific_list,
			const layer_configuration_specific& output_configuration_specific,
			const cuda_running_configuration& cuda_config) const
		{
			return layer_updater_cuda::ptr(new gradient_modifier_layer_updater_cuda());
		}
	}
}
