...
namespace nnforge
{
	namespace cuda
	{
		...
		__global__ void convolution_tex_generic_blocked_kernel_fermi(
		...
		__launch_bounds__(256, 2)
		__global__ void convolution_tex_exact_blocked_kernel_fermi(
			float * __restrict output,
			const float2 * __restrict weights,
			const float * __restrict biases,
			const packed_config<DIMENSION_COUNT> * __restrict packed_config_list,
...
#define launch_kernel(dimension_count_const, window_width, block_size) \
	switch (block_size) \
		...
		class convolution_layer_tester_cuda_fermi : public layer_tester_cuda
		{
		public:
			convolution_layer_tester_cuda_fermi()
			{
				input_tex_ref.addressMode[0] = cudaAddressModeBorder;
				input_tex_ref.normalized = false;
			}

			virtual ~convolution_layer_tester_cuda_fermi()
			{
			}
				...
				unsigned int entry_count)
			{
				...
				cuda_safe_call(cudaBindTexture(0, input_tex_ref, *additional_buffers[1], desc, additional_buffers[1]->get_size()));
