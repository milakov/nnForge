...
namespace nnforge
{
	namespace cuda
	{
			...
			int entry_count)
		{
			...
			if (thread_id == 0)
			{
				int output_offset = entry_id * top_n * elem_count_per_feature_map + neuron_id;
				for(int i = 0; i < top_n; ++i)
				{
					output[output_offset] = ((sum <= i) ? 1.0F : 0.0F);
					output_offset += elem_count_per_feature_map;
				}
			}
		}

		accuracy_layer_updater_cuda::accuracy_layer_updater_cuda()
		{
		}
			...
			unsigned int entry_count)
		{
			...
			accuracy_upd_kernel<<<dim3(elem_count_per_feature_map, entry_count), threadblock_size, smem_size, stream_id>>>(
				*output_buffer,
				*input_buffers[0],
				*input_buffers[1],
