...
#include "hyperbolic_tangent_layer_hessian_cuda.h"
...
__global__ void hyperbolic_tangent_hess_kernel(
...
__global__ void hyperbolic_tangent_square_deriviative_hess_kernel(
...
namespace nnforge
{
	namespace cuda
	{
		hyperbolic_tangent_layer_hessian_cuda::hyperbolic_tangent_layer_hessian_cuda()
		{
		}
		...
		void hyperbolic_tangent_layer_hessian_cuda::enqueue_test(
			...
			unsigned int entry_count)
		{
			...
			std::pair<dim3, dim3> kernel_dims = cuda_util::get_grid_and_threadblock_sizes_sequential_access(
				*cuda_config,
				elem_count);
			hyperbolic_tangent_hess_kernel<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
		...
		void hyperbolic_tangent_layer_hessian_cuda::enqueue_backprop(
			cudaStream_t stream_id,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& schema_data,
			const std::vector<const_cuda_linear_buffer_device_smart_ptr>& data,
			...
			unsigned int entry_count)
		{
			...
			std::pair<dim3, dim3> kernel_dims = cuda_util::get_grid_and_threadblock_sizes_sequential_access(
				*cuda_config,
				elem_count);
			hyperbolic_tangent_square_deriviative_hess_kernel<<<kernel_dims.first, kernel_dims.second, 0, stream_id>>>(
		...
		bool hyperbolic_tangent_layer_hessian_cuda::is_in_place_backprop() const
		{
			return true;
		}
