...
namespace nnforge
{
	namespace cuda
	{
			...
			int updater_entry_count)
		{
			...
			if (neuron_id < neuron_count)
			{
				...
				if (actual_val > 0.0F)
				{
					...
					gradient = actual_val * (1.0F - predicted_val);
				}
				if (actual_val < 1.0F)
				{
					err -= (1.0F - actual_val) * __logf(max(1.0F - predicted_val, 1.0e-20F));
					gradient -= (1.0F - actual_val) * predicted_val;
				}
				gradients[offset] = gradient;
			}
		...
		const boost::uuids::uuid& cross_entropy_error_function_updater_cuda::get_fusable_activation_uuid() const
		{
