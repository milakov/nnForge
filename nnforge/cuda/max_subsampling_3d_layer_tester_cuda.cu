...
struct __align__(4) zy_feature_map_config
{
...
__global__ void max_subsampling_3d_tex_kernel(
	...
	int zy_feature_map_config_count)
{
	...
	if (in_bounds && (window_x == 0))
	{
		...
		output[(((entry_id * feature_map_count + feature_map_id) * output_depth + output_z) * output_height + output_y) * output_width + output_x] = res;
	}
}
...
namespace nnforge
{
	namespace cuda
	{
		max_subsampling_3d_layer_tester_cuda::max_subsampling_3d_layer_tester_cuda()
		{
		}
		...
		void max_subsampling_3d_layer_tester_cuda::enqueue_test(
			...
			unsigned int entry_count)
		{
			...
			max_subsampling_3d_tex_kernel<<<kernel_dims.first, kernel_dims.second, smem_size, stream_id>>>(
				output,
				input,
		...
		std::vector<size_t> max_subsampling_3d_layer_tester_cuda::get_sizes_of_additional_buffers_per_entry() const
		{
		...
		cuda_linear_buffer_device_smart_ptr max_subsampling_3d_layer_tester_cuda::get_output_buffer(
			cuda_linear_buffer_device_smart_ptr input_buffer,
			const std::vector<cuda_linear_buffer_device_smart_ptr>& additional_buffers)
		{
		...
		void max_subsampling_3d_layer_tester_cuda::tester_configured()
		{
		...
		std::vector<size_t> max_subsampling_3d_layer_tester_cuda::get_sizes_of_additional_buffers_fixed() const
		{
			std::vector<size_t> res;
		...
		void max_subsampling_3d_layer_tester_cuda::fill_additional_buffers(const std::vector<cuda_linear_buffer_device_smart_ptr>& additional_buffers) const
		{
			{
