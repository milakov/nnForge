...
namespace nnforge
{
		...
		, current_sample_id(0)
	{
		...
		{
			input_buf.resize(neuron_data_type::get_input_size(transformer->get_transformed_data_type(original_reader->get_input_type()))
		...
		float * output_elems)
	{
		...
		if ((local_output_ptr != 0) && (output_elems != 0))
			memcpy(output_elems, local_output_ptr, output_buf_size);
	...
	unsigned int supervised_transformed_input_data_reader::get_sample_count() const
	{
